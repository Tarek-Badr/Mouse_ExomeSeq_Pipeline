...
${picard} SortSam SO=coordinate INPUT=${sampleID}.sam OUTPUT=${sampleID}.bam VALIDATION_STRINGENCY=LENIENT CREATE_INDEX=true

#rm ${sampleID}.sam 

${picard} MarkDuplicates INPUT=${sampleID}.bam OUTPUT=${sampleID}.dedup.bam VALIDATION_STRINGENCY=LENIENT CREATE_INDEX=true METRICS_FILE=${sampleID}.metrics.txt TMP_DIR=${tmpdir}

${GATK} BaseRecalibrator -R ${mm10} -L ${bedfile}  -I ${sampleID}.dedup.bam --known-sites ${dbsnp} --known-sites ${indelMills} -O ${sampleID}.recal.data.table

mv ${sampleID}.recal.data.table ${sampleID}.grp

${GATK} ApplyBQSR -bqsr ${sampleID}.grp -I ${sampleID}.dedup.bam -O ${sampleID}.recal.bam
